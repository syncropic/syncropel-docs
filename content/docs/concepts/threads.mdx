...
- **Task threads** are seeded from the alias and a nonce, so `SKL-0334` maps to one stable thread ID.
...
1. **Every task has a thread with real records.** If you want the ground truth on what a task did and when, read its thread. `spl debug replay SKL-XXXX` walks the records with the fold's status transitions annotated.
...
- **[The engine](/docs/concepts/engine)** — how records on threads trigger routing.
- **[Task management guide](/docs/guides/task-management)** — threads with task conventions, end to end.
- **[CEL expressions](/docs/guides/cel-expressions)** — the `thread()` function for rules that need thread state.
