...
Most records don't trigger reconcile work — system records skip it entirely to avoid cascades, already-handled records (`body.dispatch_handled: true`) skip it, decision responses (records with `body.fulfills` or `body.cancels`) skip it. What *does* trigger reconcile is typically a user record or a triggered record that asks for something.
...
Record needs routing
  ...
  │                                          emit as the decision gate for approval.
...
- **Intelligence decisions become decision-gate proposals.** The engine's reasoning doesn't silently execute — it proposes a routing rule and asks an operator to approve. Approved proposals become rules.
