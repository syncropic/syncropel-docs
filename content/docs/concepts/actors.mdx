...
When the claude CLI adapter dispatches an agent, it loads the agent's manual from `~/.syncro/<agent>-prompt.md` and the agent's memories from the memory thread, then appends both to the session system prompt. That's how an agent remembers "David doesn't want emojis in commit messages" across sessions even though the underlying LLM has no persistent memory of its own.

## Authenticators: how actors prove they're themselves

For CLI work on the local daemon, no authenticator is needed — the actor is inferred from `config.toml` or `SPL_ACTOR` env var. For HTTP API calls, an authenticator is required once the daemon has `auth.required = true` (the v0.16+ default):
