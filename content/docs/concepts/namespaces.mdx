...
| **PROJECT** | `acme-corp/payments` | A workstream within an org. | When two projects under the same org need independent budgets, autonomy settings, or rule sets. |
...
**2. Tenancy without forking the engine.** The same instance serves multiple orgs without cross-contamination. An attacker who somehow obtains an ORG-level credential for `acme-corp` cannot inject records claiming `bigcorp` as their namespace, the registry doesn't have a `bigcorp` ancestor for them to pass through.
...
The current implementation covers the **existence layer** of the narrowing guarantee: every record's claimed namespace and every ancestor must be Active in the registry. This is enforced at the HTTP boundary by the instance's `validate_namespace_for_ingest` check.

Two pieces of the namespace model are not yet implemented and are tracked as follow-up tasks:

- **Policy composition**: the YAML policy language (autonomy level, allowed primitives, effect limits, and so on) and the intersect-on-allow / union-on-deny composition rules. Today, the `policy` field on a namespace entry is opaque text. The narrowing check only validates existence + status, not capability subsets.
...
- **Use it now**: see [`spl namespace`](/docs/reference/cli#namespaces) in the CLI reference for the create/list/show/archive/delete commands.
