...
Sometimes a loop violates an assumption the system made about it — for example, the loop wraps up prematurely as it nears its context limit. The kernel emits a `core.work.compensation.v1` record naming the seam, the assumption, and the context.
...
# CLI — daemon-run with a task budget
