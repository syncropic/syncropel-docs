...
- **Remote** — the loop runs on the daemon (`spl work-loop ... --remote`, or any `POST /v1/work/loop` call). Tools are built in (no host shell); records land on the daemon and the thread is watchable from anywhere.
...
Sometimes a loop violates an assumption the system made about it — for example, the loop wraps up prematurely as it nears its context limit. The engine emits a `core.work.compensation.v1` record naming the seam, the assumption, and the context.
