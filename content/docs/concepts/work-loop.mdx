...
A conversation with [the chief of staff](/docs/get-started/the-assistant) is single-shot by default: you ask, it answers. A turn that needs tools escalates to a real loop automatically, and you can also start a governed run explicitly with `spl run`.

Two things change when delegation happens:

1. **The loop runs as *the user*, not as the chief.** It uses the user's identity, permissions, and tier — the chief steps aside for the duration. Every record the loop emits is signed by the user's actor DID.
...
Delegation is auto-classified — the chief sees a message that opens with `@agent` and starts a loop. The classifier is heuristic (verbs like "build", "audit", "find every…"); it can be tightened with an explicit `@agent` opening.
...
# (type into any assistant thread)
...
- [The chief of staff](/docs/get-started/the-assistant) is the default conversational assistant; a turn that needs tools escalates to a loop.
