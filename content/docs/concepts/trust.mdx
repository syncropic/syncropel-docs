...
```
trust(did:sync:agent:dev, code, did:sync:user:dpwanjala)      = 0.82  ← 47 reviews
trust(did:sync:agent:dev, code, did:sync:agent:director)      = 0.76  ← 31 reviews
trust(did:sync:agent:dev, ops,  did:sync:user:dpwanjala)      = 0.05  ← 2 reviews
trust(did:sync:agent:director, research, did:sync:user:dpwanjala) = 0.64
```
...
This has teeth in day-to-day use: the dev agent that closes SKL-0334 cannot approve SKL-0334. Its evidence for SKL-0334 is its claim of completion; the evidence that produces trust comes from the director agent or the human reviewing the PR.
...
- **[Patterns](/docs/concepts/patterns)** — how trust feeds pattern crystallization.
- **[The engine](/docs/concepts/engine)** — where trust is consulted during routing.
...
- **[CEL expressions](/docs/guides/cel-expressions#trust)** — using `trust()` in routing, permission, and health rules.
