...
## Trust is computed, not a stored number

A trust score is never written down and looked up. It is **computed on demand** from the log of outcomes and verdicts every time it's asked for. The records (an actor's work, and the verdicts on that work) are the source of truth; the score is a derived view over them.
...
- **History is queryable.** Because the score is a function of the log, you can ask what it *was* on a past date by recomputing it against a reference clock, the same way you'd inspect any other historical state.
- **The number can't be tampered with directly.** You don't edit a trust score; you add a verdict record, and the computed score reflects it. The only way to move trust is to produce evidence.

The rest of this page describes what goes into that computation — the verdict records, the Wilson lower bound, decay — and how the score it produces feeds routing and the dial.
...
trust(did:sync:agent:dev, code, did:sync:agent:reviewer)      = 0.76  ← 31 reviews
trust(did:sync:agent:dev, ops,  did:sync:user:alice)      = 0.05  ← 2 reviews
trust(did:sync:agent:reviewer, research, did:sync:user:alice) = 0.64
```
...
In most deployments, the `judged_by` dimension is mostly-invisible — you typically care about "how good is this actor in this domain?" and let the score aggregate across evaluators. But the dimension is there when you need it. Routing rules can ask for trust scored by a specific reviewer; audit can ask "what did *this* reviewer approve?" without double-counting.
...
3. The trust computation (on `TICK` loop) ingests the verdict and updates `(actor, domain, judged_by)` evidence.
4. The next query of `trust(actor, domain)` returns the updated Wilson score.

The approve/reject records are the observations. Nothing else generates trust evidence. Self-reported success, heuristic judgments, computed "this looks like it worked" guesses — none of these contribute. The rule is: **no verdict record, no trust delta**.

The same gate governs [work loops](/docs/concepts/work-loop). A loop ends by emitting its own outcome record — but that self-narrated "done, succeeded" is explicitly *not* trust evidence. A loop cannot grade itself. Trust moves only when a different actor reviews the loop's outcome and records a verdict with `spl work-loop-review <thread> --accept` (or `--reject`). That external verdict is the observation; it counts toward `trust(loop_actor, domain, evaluator)` exactly like a task approval does. The loop's outcome → an external verdict → the updated trust score: that chain is the only path by which a loop earns standing.

## Independence is structural, not advisory

The evaluator must be a different DID than the executor. If Alice approves her own work, the record is accepted into the log (Syncropel doesn't reject it), but the trust computation structurally discards self-verdicts. No trust is earned from self-approval.

This has teeth in day-to-day use: the dev agent that closes a task cannot approve its own work. Its evidence is its claim of completion; the evidence that produces trust comes from the reviewer agent or the human reviewing the PR.

For agent-to-agent review (`did:sync:agent:dev` reviewed by `did:sync:agent:reviewer`), independence holds trivially — two different DIDs, two separate trust chains. The evaluator's trust itself accumulates independently, which is why `trust(did:sync:agent:reviewer, review)` is a legitimate score you can query.
...
Decay is applied at computation time, not when records are ingested. Nothing happens to the underlying records as they age; only the computed score reflects decay. You can query "what was the trust score on 2025-11-01?" by recomputing it against a reference date.
