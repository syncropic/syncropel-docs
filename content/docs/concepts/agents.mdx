...
When you send the message, the chat-actor (typically [the chief of staff](/docs/get-started/the-assistant)) hands control to a new work loop **running as you, not as the chief**: your DID, your tier, your permissions. The chief steps aside for the duration. Two records open the handoff: a `core.user.message.v1` that carries your message, and a `core.work.delegation.v1` that opens the loop. When the loop ends, an outcome record closes it.
