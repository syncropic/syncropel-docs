...
- **Replay shortcut fully wired end-to-end across all 7 decision points.** Routing short-circuits on L1 match today; AITL and fold-rule contexts don't yet. Wiring is straightforward but per-context; coming in v0.18.x+.
- **Cross-instance pattern federation.** The hash-level gating is designed, the consent filter exists, but the pattern sync protocol is on the v0.19 "Discovery" track (ADR-038+).
