...
- **Pattern inspection CLI.** `spl pattern list/show` is not yet implemented — patterns are observable today only by reading the pattern index via the HTTP API. Tooling is on the polish backlog.
