...
<Callout type="info" title="v0.52: command renamed">

In **v0.52.0 "Foundational"** (2026-05-20), the `spl agent-loop`
command was renamed to **`spl work-loop`**. The behaviour is unchanged —
only the name shifted, in keeping with the new framing of the primitive
as a goal-structured *work loop* that any actor can drive.

The old name returns "unknown subcommand" — scripts that called
...
## Federation — Pair Lifecycle (canonical, v0.24+)
...
Pre-v0.24 surface for federation pair management. Still functional but **superseded by `spl federation`** above for new pairs. See the [federation guide](/docs/guides/federation) for legacy walkthrough.
