...
Wipes `~/.syncro/` back to a fresh state. Refuses to run while the daemon is up — stop it first with `spl serve --stop`. Backs up `hub.db` to `~/.local/share/syncropel/backups/hub.db.pre-reset.<iso8601>` (outside `~/.syncro/` so the backup survives the reset) before deleting anything. Never touches `~/.syncro-data/` (task content + alias mappings) by default.
