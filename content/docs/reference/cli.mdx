...
## Agent loop — `spl agent-loop`

Run a records-native agent loop with workspace tools. The loop emits records onto its thread as it works (`core.agent.turn.v1`, `core.agent.tool_call.v1`, etc.); the kernel decides terminal success in `--verify` mode. See the [Agent loop concept page](/docs/concepts/agent-loop) for the full primitive.
...
spl agent-loop "Find every record tagged 'beta' and emit consent grants" \
  --remote --verify --json
```
