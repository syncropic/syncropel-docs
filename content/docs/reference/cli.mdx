...
| `serve --insecure-localhost` | Force-bind to `127.0.0.1` and disable auth enforcement (recovery only; WARN-logged) |
...
Creates a dedicated service account, mints a token, and renders both as a QR code in the terminal (plus a plain-text fallback). Used to connect phones, browsers, and other devices to a `spl serve` daemon.

| Flag | Description |
|---|---|
| `--device "<label>"` | Device name (e.g. `"iPhone 17"`, `"browser-chrome"`) — required |
| `--url "<remote_url>"` | URL the device will call — required. Must be reachable from the device |
| `--scopes <csv>` | Token scopes (default `records:read,records:write`) |
