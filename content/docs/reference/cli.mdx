...
<Callout type="info" title="v0.52 command rename">

In **v0.52.0 "Foundational"** (2026-05-20), `spl agent-loop` was renamed
to `spl work-loop` per [ADR-125](/docs/reference/spec). Hard cutover —
...
Run a records-native agent loop with workspace tools. The loop emits records onto its thread as it works (`core.work.turn.v1`, `core.work.tool_call.v1`, etc.); the kernel decides terminal success in `--verify` mode. See the [Agent loop concept page](/docs/concepts/agent-loop) for the full primitive.
