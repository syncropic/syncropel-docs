...
spl stop             # stop the running daemon
...
`spl start` / `spl stop` / `spl restart` are convenience aliases over the daemon-mode `spl serve` flags. Use them for interactive work. Use `spl serve` (foreground) under process supervisors that expect the daemon to stay attached for log capture + restart-on-crash semantics — systemd `Type=simple` units, Docker `CMD`, Kubernetes pods, and managed cloud platforms.
...
Refuses if any of four signals indicate a daemon is running: PID file at `~/.syncro/run/spl.pid`, Unix socket connect, TCP `127.0.0.1:9100` connect, or `systemctl --user is-active spl-serve.service`. The TCP and systemd signals are scoped to the default `~/.syncro/` home so `SYNCROPEL_HOME=/some/other/path` invocations don't false-positive on unrelated daemons. If you use a systemd user unit, stop it (`systemctl --user stop spl-serve.service`) — `spl stop` alone is insufficient because the unit respawns the daemon.
...
| `chrome validate <file>` | Schema-check a `core.instance.shell.v1` body in a local file. No daemon needed — good for pre-commit hooks. Exit `0` on valid, `1` on invalid |
...
| `--remote` | Run the loop on the daemon (built-in tools, no host `bash`) instead of in-process. Records land on the daemon and the thread is watchable from anywhere. |
...
| `--token <BEARER>` | Bearer token for daemon auth. Precedence: `--token` > `SPL_TOKEN` env > `~/.syncro/token`. |
...
# Daemon-run, verified, JSON output
...
Find other daemons without hand-configured peer URLs. See the [federation pair guide](/docs/guides/federation-pair) for the canonical pairing flow once a peer is found, and the [federation discovery guide](/docs/guides/federation-discovery) for when each discovery method fits.
