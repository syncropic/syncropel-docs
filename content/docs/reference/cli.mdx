...
This section targets v0.11.0+ multi-instance deployments. The commands below do not exist in v0.10.x.
</Callout>

Multi-instance coordination primitives from ADR-021 (instance registry + kill switch) and ADR-022 (task fan-out + barrier). See [Parallel Dev Tutorial](/docs/tutorials/parallel-dev) for a hands-on walkthrough and [Operator Runbook: Fleet Operations](/docs/operate/runbook#multi-instance-fleet-operations) for day-2 procedures.
...
- **`budget`** (optional) — advisory per-subtask USD budget (not enforced in v0.11; Phase 3/v0.12+ enforces)
...
<Callout type="info">
v0.11 does not support declaring subtasks via a task body file. The `--subtask` CLI flag form is the only shape shipped. A future release may add body-file parsing as an alternative form.
</Callout>
...
Custom strategies (e.g. trust-weighted selection) are a Phase 3 concern.

## Federation — Pair Sync (v0.13.0+)

Continuous record sync between two daemons per ADR-027. See the [federation guide](/docs/guides/federation) for walk-through and semantics.
...
Uses [ADR-023 thread snapshot/restore](https://github.com/syncropic/syncropel-spec) under the hood. Useful for onboarding a new instance with existing history.

## Federation — Consent (v0.13.0+)

Grant and manage cross-namespace record-sharing per ADR-024 / Frozen Foundation F8. See the [consent guide](/docs/guides/consent).
...
Source namespace is `default` in v0.13.0. Without L0 in `--hash-levels`, records are downgraded to `{"redacted": true}` when they cross the boundary.
