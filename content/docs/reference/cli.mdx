...
Run a records-native work loop with workspace tools. The loop emits records onto its thread as it works (`core.work.turn.v1`, `core.work.tool_call.v1`, etc.); the kernel decides terminal success in `--verify` mode. See the [Work loop concept page](/docs/concepts/work-loop) for the full primitive.
...
| `--remote` | Run the loop on the daemon (substrate-native tools, no host `bash`) instead of in-process. Records land in the substrate and the thread is watchable from anywhere. |
