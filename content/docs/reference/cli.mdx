...
```bash
spl task fan-out <task-alias>                   # Fan out from task body.subtasks
spl task fan-out <task-alias> \
  --subtask 'goal="...",target=worker-a,budget=2.00' \
  --subtask 'goal="...",target=worker-b,budget=2.00' \
  --join all                                    # Explicit descriptors
...
`spl task fan-out` emits a parent INTEND on the coordinator with the subtask list. The fan-out reconciler spawns one child INTEND per descriptor and POSTs each to its target worker's `/v1/records` endpoint via the instance registry.

The `--join` flag (or `body.join` in the task file) accepts shorthands (`all`, `any`, `k_of_n:K`) or a custom CEL expression evaluating against the `children` binding. Examples:
