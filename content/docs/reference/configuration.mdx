...
Managed via `spl config embedding-provider set|show|clear` (see the [CLI reference](/docs/reference/cli#embedding-provider)). Both commands write a LEARN record on `th_engine_config`; the daemon rebuilds its embedder on broadcast — no restart.
