...
|---|---|---|
| GET | `/health` | Health check — returns `{"status": "ok", "version", "instance_did", "proof_kinds_accepted"}`. `proof_kinds_accepted` lists which re-credential proofs this instance takes (`recovery_code`, and `account` or `passkey` once declared), so a client can tell before authenticating. |
