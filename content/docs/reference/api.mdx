...
resolved at start), `transport`, and the tool-call tally
(`tool_calls_ok`, `tool_calls_failed`). A definition whose prompt does not
resolve refuses the start with `422 WORK_PROMPT_UNRESOLVED`, naming the
record. An open ask on the row names its `proposal_kind` when it is a
proposal.
