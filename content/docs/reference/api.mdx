...
| Method | Path | Description |
|---|---|---|
| POST | `/v1/work/loop` | Start a work loop. Body: `{goal, max_turns?, verify?, workspace?}`. Returns the loop's `thread` id. |
...
Returns the new thread id, the resolved tier, and an `effective_config` block carrying the wall-clock, token budget, and verification mode the loop will run under. Watch progress via the stream endpoint or query the thread directly.
