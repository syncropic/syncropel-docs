...
A fold is a derived view computed from records — the turn timeline, thread state, trust, and more. One endpoint resolves any fold; the kernel dispatches by name.
...
Graph queries traverse the substrate as a graph of people, threads, and the records that connect them. Pass `?root=⊤` to query across everything you reach (see [Universal traversal](/docs/guides/universal-traversal)).
