...
definition record the run executed under) and `prompt` (the prompt record
it resolved at start). A definition whose prompt does not resolve refuses the
start with `422 WORK_PROMPT_UNRESOLVED`, naming the record.
