...
Two response fields are easy to misread. `capped: true` means more disclosable rows may exist beyond this page, whatever `limit` you asked for; it is the only truthful signal that a page is partial. `matched_total` is the filter's count before disclosure, so it is not comparable with `data.length` and must not be used to decide whether you have everything; an authorized reader can receive fewer rows than `matched_total` on a complete read.
...
| POST | `/v1/decisions/:id/decide` | Approve or reject a pending decision |
...
**Membership envelope (ADR-194).** An optional `membership` object turns the invite principal-creating — redemption mints an identity + membership grant instead of a bare credential:
...
| `429` | `RATE_LIMITED` | Re-credential attempts exceeded the burst (3) and refill (one per 5 s) for that principal string or address |
...
The membership roster + offboarding surface (ADR-194). Concept: [Principals & grants](../concepts/principals-and-grants). Operator walkthrough: [Add a teammate](../tutorials/add-a-teammate).
