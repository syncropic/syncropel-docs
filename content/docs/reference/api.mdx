...
In **v0.52.0 "Foundational"** (2026-05-20), the agent-loop HTTP surface
was renamed from `/v1/agent/*` to `/v1/work/*` per
[ADR-125](/docs/reference/spec). Error codes `AGENT_LOOP_*` → `WORK_*`.
Hard cutover — the old paths return 404. v0.51 clients must update.
...
| Method | Path | Description |
|---|---|---|
| POST | `/v1/work/loop` | Start an agent loop. Body: `{goal, max_turns?, verify?, workspace?}`. Returns the loop's `thread` id. |
