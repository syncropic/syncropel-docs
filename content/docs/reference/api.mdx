...
## Capabilities manifest (v0.13.0+)

| Method | Path | Description |
|---|---|---|
| GET | `/.well-known/syncropel` | Progressive-enablement manifest for clients |

Returns the kernel version, tier, and a map of boolean capability flags. Clients use this to enable/disable features based on what the connected daemon actually supports. Advertised capabilities in v0.13.0 include `sync`, `directory` (gated by `config.directory_enabled`), `federation_consent`, `identity`, `dispatch_observability`, `cusum_alerts`, `trust_detail`, and `reconcile_queue`.
...
Returns `{actor, display_name, did, method, key_path, key_fingerprint}`. Populated from `~/.syncro/config.toml` at daemon startup. New installs from v0.13.0 generate a `did:key:...` on `spl init`; older installs need `spl init --force` once to upgrade.

## Federation sync (v0.13.0+)

Federation is ADR-027 "The Pair" — pull-first HTTP replication between two daemons. Both flat (`/v1/sync/*`) and domain-grouped (`/v1/federation/sync/*`) routes are served.
...
**Known limitation (v0.13.0)**: `SyncRegistry` is in-memory. Pairs are lost on daemon restart and must be re-created. Persistence is on the v0.13.1 roadmap.

## did:sync directory (v0.13.0+)
...
Gated by `config.directory_enabled` — off by default. A daemon can act as a did:sync directory provider; most installs don't need to. See [ADR-028 Federated Identity](https://github.com/syncropic/syncropel-spec) for the directory protocol design (Bluesky did:plc-inspired).
