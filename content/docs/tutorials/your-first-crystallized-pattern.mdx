...
- An instance running with `spl status` healthy.
- Completed [Your first decision gate](./your-first-decision) — patterns build on top of approved rules.
...
If no rules exist yet for this shape, Syncropel will treat the new INTEND like the unrouted record from [the decision-gate tutorial](./your-first-decision) — proposing a routing rule as a decision request for you to approve. The pattern is observable but no auto-routing happens until a rule covers the shape.
...
- [Cookbook: Trust-gated auto-approval](/docs/cookbook/trust-gated-auto-approval) — composing trust + the decision gate into automatic decisions for high-trust actors.
...
| Fourth record didn't auto-route | No routing rule covers the pattern's L1 signature yet. Patterns make routing *possible*; rules make it *fire*. | Either approve a routing proposal (per [the decision-gate tutorial](./your-first-decision)) or hand-author a rule with `spl config add-rule`. Patterns and rules are orthogonal — the cascade matches patterns; rules dispatch. |
