...
<Callout type="info">
Requires **Syncropel v0.11.0 or later**. If you want the full hands-on walkthrough with real work and mid-run failures, go to [Parallel Dev — Monday Morning Walkthrough](/docs/tutorials/parallel-dev). If you just want to see what a fan-out *looks like*, you're in the right place.
</Callout>
...
Fan-out speedup report: th_abc123...
  ...
  Weak estimate (single-run, sanity check only):
    ...
    caveat:            research/09 §1 option C — useful as a sanity check,
                       not a methodology-compliant measurement. For release-
                       gate drills, pass a matched serial baseline via
                       `--vs-serial <thread>` (path B, concurrent matched).
```
...
You exercised the four primitives that make up v0.11.0's Fleet release:

1. **Instance registry** (ADR-021) — `spl fleet list` shows you the registered instances.
2. **Fan-out** (ADR-022) — `spl task fan-out` writes one parent INTEND with a subtask descriptor list, and the engine's fan-out handler spawns child INTENDs on the targeted workers.
3. **Barrier join** (ADR-022) — the parent's completion waits until the join predicate (`all`, `any`, `k_of_n`, or any CEL expression) is satisfied by the child completions.
4. **Speedup measurement** (v0.11 addition) — the `spl task speedup` tool reads `fanout_child_done` records and reports the ratio you care about.

## Next steps

- **[Parallel Dev — Monday Morning Walkthrough](/docs/tutorials/parallel-dev)** — the real hands-on version: non-trivial subtasks, mid-run failure, kill switch recovery, ~20 minutes.
- **[Fleet Benchmarking](/docs/operate/fleet-benchmarking)** — the operator guide for running release-gate bootstrap drills and interpreting speedup ratios.
