...
description: Two daemons paired in one command, code-review records flowing both ways consent-gated. The complete two-person federation walkthrough.
---

By the end of this tutorial, two daemons — call them **alice** and
...
Both should report a healthy instance. You now have two daemons ready
to be paired.
...
(Substitute alice's DID — `spl identity show` on alice's daemon
...
- A code-review flow that's *multi-user by design* — two roles, two
  daemons, distinct publishing patterns on each side.
