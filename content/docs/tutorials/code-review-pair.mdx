...
description: Two daemons, one shared workspace template, records flowing both ways consent-gated. The complete two-person walkthrough using the bundled code-review-pair template.
---
...
Both alice and bob install the same template. The template scaffolds
a workspace declaring three threads and three view components. Each
side gets their own copy.
...
You now have two daemons, each with their own copy of the
...
- A workspace template that's *multi-user by design* — two roles
  with distinct publishing patterns, both sharing the same view
  components.
...
- Records flowing in both directions consent-gated. Alice's request
  ...
  unless you grant it.
- The same template that anyone can install via `spl workspace init`
  to start the same flow with their own collaborators.
