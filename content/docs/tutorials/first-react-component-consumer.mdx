...
<Callout type="info">
This tutorial assumes you've completed [Your First SDK Integration](/docs/tutorials/first-sdk-integration) and have a `spl` instance running locally with a token. The React palette has no provider and no required CSS framework — it works with any React 18+ app.
</Callout>
...
Browser code can't read `~/.syncro/token` directly. Put the instance-facing logic in a Next.js API route so the token stays server-side.
...
- Server-side instance access via an API route (token never reaches the browser)
...
- ✅ Reads the bearer token server-side and proxies to the instance
