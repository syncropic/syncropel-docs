...
description: 15-minute walk from npm install to a working Node.js script that emits records, attaches canonical references, and queries them back. Zero magic — just the engine speaking JSON over HTTP.
---

<Callout type="info">
This tutorial assumes you already have **`spl` installed and an instance running** with a token in `~/.syncro/token`. If you don't, do the [Quickstart](/docs/start) and [Authentication setup](/docs/operate/keyring) first — both take under five minutes.
</Callout>
...
4. Queries the thread back and prints what the engine saw
...
- `spl serve --status` reports the instance as running
...
That's the record log: an immutable, content-addressed history of what your app intended, did, and concluded.
...
Reading back uses the same client. Two methods:
...
The query is **fail-open**: if the instance is unreachable or returns 5xx, you get an empty array, not a thrown exception. That's deliberate — your app keeps responding even when the engine is degraded. The same property holds for `emit`: transport errors return `{ success: false, error }` instead of raising. Grammar errors (a malformed `body.kind`) still throw, because they are programmer mistakes the engine can't fix.

## What you have now

A Node script that:

- ✅ Authenticates against a local Syncropel instance
