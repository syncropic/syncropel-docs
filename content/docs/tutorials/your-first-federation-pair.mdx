...
`--insecure-localhost` tells each instance to skip TLS verification on cross-instance calls. Use it for **local two-instance dev only**. For real cross-host pairs, both sides should serve TLS.
