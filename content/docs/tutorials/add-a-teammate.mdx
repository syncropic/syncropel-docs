...
description: "The golden path for membership: issue an invite that creates a real identity, your teammate redeems it in the browser, works under their own signed identity, and offboarding is one command that kills everything they were ever handed."
---

## What you'll do

You run a workspace. A teammate, call her Mora, needs to work on it. In this tutorial you will:
...
**Prerequisites**: a running workspace you hold an `admin` bearer for, reachable at a URL your teammate's browser can open.
...
Send Mora the link over whatever channel you trust. The URL is signed, so it is tamper-evident, but anyone holding it before it expires can redeem it, so treat it like a key, not like a webpage.

## 2. Your teammate redeems it

Mora opens the link. The invite page shows who issued it, what scopes it grants, and asks her to:

- **Choose a member name**: lowercase letters, digits, and hyphens, up to 63 characters (`mora`, `mora-k`). It becomes her DID on your workspace: `did:sync:user:mora`.
- **Accept custodial custody**: the page states plainly that your workspace will hold her signing key and sign on her behalf. That is the only custody mode today; [Joining a workspace](/docs/get-started/joining-an-instance) covers what it means from her side.

She confirms. Behind the button, the browser calls `POST /v1/invites/inv_9f2c41d0b8a7/redeem` with her `member_label` and `custody: "custodial"`, and your workspace writes the record spine (identity genesis, the label binding, the membership grant, and the custodian-signed acceptance), then mints her a service account and bearer parented to the grant. The page hands her the bearer once.
...
Note the visibility rule: **you see every row because you are admin.** When Mora runs `spl member list` against your workspace, she sees only her own row. The grant graph at `GET /v1/folds/grant` scopes the same way: admin sees everything, everyone else sees only grants where they are the issuer or the audience.
...
The verdict record names her as the evaluator, carries her signature, and the workspace verifies it because her label binding resolves to a key it knows. A verdict with a wrong signature is refused. And she cannot approve her own work through an alias, because self-judgment is refused by resolving both parties to their principals through the grant chain, not by comparing DID strings.
...
She redeems the new link exactly as before. Because the invite is pinned to a label your workspace holds custody for, redemption becomes a **renewal**: the same principal (same DID, same history, same trust) gets a fresh grant and a fresh bearer, and the old grant is revoked with reason `membership-renewed`.
...
- **Bilateral.** Mora can revoke her own membership too, because leaving is not something only the operator can do.
- **Idempotent.** Revoking an already-revoked member reports success without erroring.
- **`scope: local`.** The revocation is enforced on *this* instance's ledger. Credentials that live on other workspaces are outside its reach.
...
- **[Joining a workspace](/docs/get-started/joining-an-instance)**: the page to send your teammate alongside the link.
