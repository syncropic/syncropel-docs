...
- **On Windows**: Unix-socket trust is not yet wired for the Windows local-CLI path. Create a bearer with [`spl pair --device local-windows --url http://127.0.0.1:9100 --scopes admin`](/docs/get-started/pairing) and save the printed token.
...
## Corrupted record store or stuck locks
...
Any records you had are gone. Automatic backups under `~/.local/share/syncropel/backups/instance-<did-tail>/` (a per-instance directory keyed by the instance DID) may contain a recent snapshot: see [Operator runbook → Recovery](/docs/operate/runbook) for the full restore procedure.
...
These apply to a workspace created at syncropel.com.

- **A connection error right after a quiet period.** The workspace was asleep and is waking up. Waking usually takes a couple of seconds; try again in a moment.
- **A `401` after reconnecting a machine.** The key this machine holds is no longer accepted. Connect the machine again and save the new key with `spl token save <token>`; see [Connect your command line](/docs/get-started/hosted#connect-your-command-line-to-the-hosted-workspace).
- **Storage full on the free plan.** A free workspace has 1 GB of storage; a storage-full error means it has reached that limit. See [Pricing](/docs/get-started/pricing) for what each plan includes.
