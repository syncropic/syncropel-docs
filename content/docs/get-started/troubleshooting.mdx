...
Fail-closed guard is doing its job. You haven't bootstrapped the first service account yet. Follow the four-step flow on the [Service accounts page](/docs/get-started/service-accounts#the-bootstrap-flow) — start the daemon in `--insecure-localhost`, bootstrap the SA, save the token, restart in secure mode.
...
Accept the firewall prompt (or add a rule manually — see [Windows notes](/docs/get-started/windows#windows-firewall)). The install proceeds once the binary is allowed to bind.
...
- [Windows notes](/docs/get-started/windows) — WSL, Defender, Firewall, PATH, long paths
