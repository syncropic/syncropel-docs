...
description: One identity can own several Syncropel instances — each a full runtime with its own stable URL, all owned by you. This page explains the ownership model, instance URLs, and how to reach each one.
---
...
A signed-in identity can create as many instances as it needs; each is billed on its own (see [Pricing](/docs/get-started/pricing)). Each one is a real, full runtime, not a reduced version of the product.
