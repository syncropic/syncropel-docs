...
`spl config scribe enable` shows a short privacy disclosure and turns on
the default conversational actor for new threads; `spl config scribe
status` shows what's configured, and `disable` turns it back off. If you
initialized bare and the assistant seems silent, this is why — nothing is
broken.
