...
`spl start` / `spl stop` / `spl restart` are the convenience verbs for interactive operator use; the full `spl serve` form (foreground) is the canonical entry point used by systemd units, Docker containers, Kubernetes pods, managed cloud platforms, and other process supervisors.
</Callout>
