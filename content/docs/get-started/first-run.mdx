...
```bash
spl init --preset recommended  # at init: seed a ready-to-use setup
                               # (presets: recommended · blank)

spl blueprint install chief-of-staff   # any time later: install the
                                       # conversational actor on an
                                       # already-initialized instance
```

Every preset except `blank` installs the **chief of staff**, the actor that
answers on a new thread. On an instance that was initialized bare, install it
through the blueprint door with the command above. If you initialized bare and
the assistant seems silent, this is why — nothing is broken.

<Callout type="info">
Instances before v0.216 had a separate built-in assistant, configured with
`spl config scribe`. That verb is gone: the conversational default is now an
ordinary actor, defined by a blueprint like any other, so what answers you is
