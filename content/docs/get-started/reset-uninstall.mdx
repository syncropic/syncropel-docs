...
```bash
spl serve --stop            # stop the daemon first (reset refuses if it is running)
...
What `spl reset` deletes by default:

| Path | Why |
|---|---|
| `hub.db` (+ `-wal`, `-shm`) | The records database — tasks, trust, dispatch history, routing rules |
| `keys/` | Identity keypair — you get a new DID after `spl init` |
| `secrets/` | Encrypted API keys + secret-backend store |
