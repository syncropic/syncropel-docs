...
### First boot after reset (v0.65.0+)

As of v0.65.0 the canonical post-reset sequence is two commands and you're done:

```bash
spl init           # generates identity + config (mode 0640)
spl serve --daemon # daemon binds Unix socket + TCP loopback
# Verify
spl status   # daemon healthy
spl doctor   # all green
```

`spl task add "..."`, `spl thread list`, `spl status`, and every other CLI command works immediately — no service account, no bearer token, no `--insecure-localhost` dance. The local CLI authenticates via the Unix socket (`~/.syncro/run/spl.sock`, mode 0700 owned by your UID); filesystem permissions ARE the authentication.

You only need to mint a service account when **delegating** scoped access to a different principal — browser pair, paired phone, MCP client, remote CLI, federation peer. See [Service accounts and tokens](/docs/get-started/service-accounts).

If you're on v0.64.x or older, the legacy `--insecure-localhost` + `service-account create --bootstrap` + `token save` + restart-secure dance still works. Upgrade to v0.65+ to skip it.
