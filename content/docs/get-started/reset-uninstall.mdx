...
Automatic backups under `~/.local/share/syncropel/backups/instance-<did-tail>/` (a per-instance directory keyed by the instance DID) are written outside `~/.syncro/`, so they survive **reset but not full disk cleanup**. The backup is destructive. Every instance start overwrites it. If you care about your records, copy them off-host before running either of the operations on this page.

## Clean reset with `spl reset`
...
The manual `rm -rf` does NOT auto-back up `hub.db` first, so if there's anything you want to keep, copy it off-host before running this. The instance-startup backup at `~/.local/share/syncropel/backups/instance-<did-tail>/hub.db.bak` is your last line of defense after the fact.
...
`spl task add "..."`, `spl thread list`, `spl status`, and every other CLI command works immediately, with no service account, no bearer token, no `--insecure-localhost` step. The local CLI authenticates via the Unix socket at `~/.syncro/run/spl.sock` (mode 0700 owned by your user); filesystem permissions are the authentication.

You only need to create a service account when **delegating** scoped access to a different principal: a browser pair, a paired phone, an MCP client, a remote CLI, or another instance you sync with. See [Service accounts and tokens](/docs/get-started/service-accounts).
...
- Records that were synced to another instance before the reset or uninstall. Those live on the remote peer independently.
...
The directory is keyed by the 24-hex tail of the instance DID. After a reset the instance that wrote it is gone, so just list what's there:

```bash
ls ~/.local/share/syncropel/backups/*/
```
