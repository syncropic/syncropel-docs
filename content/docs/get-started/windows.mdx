---
title: Windows Notes
description: Platform-specific guidance for Windows, covering WSL gotchas, Windows Defender, Windows Firewall, user-scope PATH, and long path names.
---
...
Windows Defender can scan `hub.db` on every read or write, which slows down a busy instance. It also occasionally flags fresh release binaries as "unknown" while reputation data catches up, which triggers an interactive prompt on first run.
