...
Your CLI talks to your workspace via `~/.syncro/token`. Browsers and phones can't read that file, so they need their own service account and token. `spl pair` wraps the whole "mint a scoped account, render a token, show it in a form the other device can consume" flow in one command.
