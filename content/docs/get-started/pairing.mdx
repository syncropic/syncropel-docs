...
A fresh browser pointed at `syncropel.com` has no way to authenticate to your local instance. It doesn't know what instance URL to hit, and even if it did, it has no token. Three ways to solve this:
...
Click it (or copy it into the browser you want to pair). The page reads the encoded payload, verifies the token against your instance, stores the credential in the browser, and drops you on `/tasks` — no typing, no scanning, no copy/paste.
...
`syncropel.com` distinguishes a small set of connection states so you can tell at a glance what your instance wants from you:
...
`syncropel.com` on mobile no longer probes `localhost:9100` — phones can't reach an instance there, and the resulting browser-blocked error confused more than it helped. Mobile users now land on a welcome screen with three explicit paths:
