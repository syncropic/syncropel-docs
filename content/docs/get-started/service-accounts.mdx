...
The local CLI talks to the daemon over the Unix-domain socket at `~/.syncro/run/spl.sock` — filesystem permissions (the socket is mode 0700 owned by your user) are the authentication. The full first run is `spl init && spl serve && spl task add "..."`. See [First run](/docs/get-started/first-run).
