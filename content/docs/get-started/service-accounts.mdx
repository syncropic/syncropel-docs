...
If you're on a instance that does not support Unix-socket trust, or you've explicitly disabled it via `[auth] unix_socket = false` in `config.toml`, the original bootstrap flow still works:
