...
The local CLI talks to the daemon over the Unix-domain socket at `~/.syncro/run/spl.sock` — filesystem permissions (the socket is mode 0700 owned by your user) are the authentication. The full first run is `spl init && spl start && spl task add "..."`. See [First run](/docs/get-started/first-run).
...
|---|---|
| Local CLI on Linux/macOS, talking to local daemon | Unix-socket trust — no service account needed |
| Local CLI on Windows | Use a bearer token (mint one with the `spl pair` flow below) |
| Browser paired with a local daemon | [`spl pair --device browser-<name>`](/docs/get-started/pairing) — mints a scoped browser bearer |
...
| Anonymous web ingest (public form posting to your daemon) | Set the namespace `write_policy = "anonymous"`; see [exposing your instance](/docs/get-started/exposing) |

## Minting a service account for delegation

Works on any running daemon, no special mode required.
...
If you're on a daemon that does not support Unix-socket trust, or you've explicitly disabled it via `[auth] unix_socket = false` in `config.toml`, the original bootstrap flow still works:

```bash
spl start --insecure-localhost
