...
### Legacy bootstrap dance (rarely needed)

If you're on an instance that does not support Unix-socket trust, or you've explicitly disabled it via `[auth] unix_socket = false` in `config.toml`, the original bootstrap flow still works:
...
For the canonical first-run flow, you can skip these steps entirely.
