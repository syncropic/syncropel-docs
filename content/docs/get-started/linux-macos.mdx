...
description: Platform-specific guidance for Linux and macOS, including systemd user units, launchd, PATH setup, and where logs live.
---
...
```ini
[Unit]
Description=Syncropel instance (user service)
...
User services only run when the user is logged in by default. For an instance that comes up at boot without login, enable lingering:
...
On distros without systemd (Alpine, many embedded or minimal images), start the instance from your shell's rc file or a startup script:
...
Log rotation is automatic at 10 MB per file, keeping the last 7 rotations at `~/.syncro/logs/spl.log.1` through `spl.log.7`. The on-disk total tops out near 80 MB even on a very busy instance.
...
Automatic backups are written outside this directory, under `~/.local/share/syncropel/backups/instance-<did-tail>/` (keyed by the instance DID). See [Operator runbook](/docs/operate/runbook) for backup discipline.
