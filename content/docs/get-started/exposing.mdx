---
title: Exposing the Daemon Securely
...
By default `spl start` binds `127.0.0.1` and is unreachable from anywhere else on your network. You'll want to change that when:
...
```bash
spl start                                 # stays on 127.0.0.1:9100
```
...
- **`syncropel.com` calling your daemon.** Add `https://syncropel.com`.
...
Once the daemon is reachable on a stable URL, a federation peer can pair to it. The pairing flow mints reciprocal `federation:manage`-scoped service accounts, exchanges public keys, and persists a pair record on both sides. See the [Federation pair guide](/docs/guides/federation-pair) for the canonical `spl federation pair <peer-url>` walkthrough and the [Discovery guide](/docs/guides/federation-discovery) for auto-discovery via DNS or mDNS.
...
| Browser on `syncropel.com` | Loopback daemon + CORS allowlist |
...
- [Federation pair guide](/docs/guides/federation-pair) — pair two daemons for bi-directional sync via `spl federation pair`
- [Operator runbook](/docs/operate/runbook) — backup discipline, recovery, upgrades
- [Troubleshooting](/docs/get-started/troubleshooting) — when an exposed daemon isn't reachable or CORS is blocking
