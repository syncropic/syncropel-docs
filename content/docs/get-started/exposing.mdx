...
Once the daemon is reachable on a stable URL, a federation peer can pair to it. The pairing flow mints a `federation:manage` scoped service account, exchanges public keys, and sets up a sync subscription. See the [Federation guide](/docs/guides/federation) for the full flow and the [Discovery guide](/docs/guides/federation-discovery) for auto-discovery via DNS or mDNS.
...
- [Federation guide](/docs/guides/federation) — pair two daemons for bi-directional sync
