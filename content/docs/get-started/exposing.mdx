...
Once the instance is reachable on a stable URL, another workspace can pair with it. The pairing flow creates reciprocal `federation:manage`-scoped service accounts, exchanges public keys, and persists a pair record on both sides. See the [workspace pairing guide](/docs/guides/federation-pair) for the canonical `spl federation pair <peer-url>` walkthrough and the [Discovery guide](/docs/guides/federation-discovery) for auto-discovery via DNS or mDNS.
...
- [Workspace pairing guide](/docs/guides/federation-pair): pair two instances for bi-directional sync via `spl federation pair`
