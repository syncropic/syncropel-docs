...
`validateSRP` walks every node, checks required fields, and reports structured errors with `path`, `code`, and `message`. Use it on any input that claims to be SRP — records you received from an instance, documents an AI generated, user-submitted JSON.
...
Projections are most useful when they travel as record bodies — e.g., an instance emits a KNOW record whose body *is* a projection document, and the UI queries that thread and renders the documents.
