...
awake. A trigger that falls due while the workspace sleeps is handled by its `catchup` rule the
next time the workspace wakes, for example when you open it.

Waking a sleeping hosted workspace at the moment its next trigger falls due is built and is being
switched on for hosted workspaces. Until it is on, do not rely on a schedule firing on time while
nobody is using the workspace.
