...
<Callout type="info">
**Since v0.9** (CEL-enabled since v0.12). Schedule + CEL both supported.
</Callout>
...
at 9am" pattern. No external scheduler needed; the kernel's CRON loop
evaluates schedules on every tick.

## Prerequisites

- Running `spl serve` daemon (v0.14+)
