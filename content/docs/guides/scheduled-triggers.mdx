...
A hosted workspace sleeps when nobody is using it. It publishes when its next scheduled trigger
falls due, and the service wakes it at that moment. The wake carries no instruction: the workspace
checks it is a time it published, runs whatever is due from its own records, and stays awake until
