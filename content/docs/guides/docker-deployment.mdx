...
- **Reproducible E2E testing** — clean container per run, no risk of stale instances or leaked PID files between tests.
...
The simplest deployment — one instance, one volume, one host port:
...
- Reports a HEALTHCHECK every 10 seconds against `/health`. `docker ps` shows `(healthy)` once the instance is ready.
...
| `SPL_ACTOR` | Override the instance's identity actor (also settable via `config.toml`). |
...
docker exec spl spl stop && docker restart spl
```
...
Host ports are in the 9200 range so they don't collide with a production instance that may be running on `localhost:9100`. Inside each container the instance still listens on its own port 9100 — only the host-side mapping changes.
...
- **Reverse proxy with TLS**: terminate HTTPS at nginx/caddy/traefik in front of the container. The image only speaks HTTP on port 9100 by design — TLS is a deployment concern, not an engine concern.
...
- **Permission enforcement**: enable CEL permission rules before exposing the instance — see [Permission Enforcement](/docs/guides/cel-expressions#permission-enforcement) and author an admin allow rule first to avoid the lockout trap.
...
The engine may be slow to bind on first run while it creates the SQLite WAL. Wait 30 seconds and re-check. If it's still unhealthy, run the binary in the foreground to see the error:
