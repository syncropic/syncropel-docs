...
The Dockerfile lives at the root of `syncropel-core`:

```bash
git clone https://github.com/syncropic/syncropel-core
cd syncropel-core
docker build -t syncropic/spl:dev .
```

First build pulls the Rust toolchain image and runs `cargo build --release`, which takes 5–10 minutes depending on CPU. Subsequent builds reuse the cargo dependency layer and finish in under a minute when only source files change.

To build a tagged release image:

```bash
docker build -t syncropic/spl:0.9.1 .
docker tag syncropic/spl:0.9.1 syncropic/spl:latest
```
