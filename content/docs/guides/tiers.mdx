...
A workspace with no tier record for a tier yields the constant, byte-identical
...
- `past_due` is not a cut-off. A failed card narrows what a workspace may
  spend; it does not take the workspace away. The person keeps their records,
...
On a hosted workspace every run follows the limits of the workspace's plan: a free workspace runs
at the Trial ceilings, a paid one at the Paid ceilings. That holds for every way a run starts: a
message you send, an assistant stepping into a conversation, a scheduled trigger, a delegation. The
plan is set when the workspace is created, and a workspace on your own machine has no plan ceiling.
...
entitlement door accepts only a request signed by the hosting service
(`ENTITLEMENT_UNSIGNED` otherwise). The Trial ceiling allows two runs at once, so a scheduled run
