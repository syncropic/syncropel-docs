...
Start a Syncropel daemon locally (`spl start`), then:
...
- **Folds & live threads** — `client.folds.resolve()` reads any kernel fold; `client.threads.watch()` streams a thread live
...
- **Fail-open reads, surfaced errors** — record reads fail open (a flaky daemon never crashes your UI); malformed requests and data-plane writes raise so a dropped write is never mistaken for success
...
| `client.folds` | `.resolve<V>(name, cacheKey?)` reads any kernel fold through one typed method; `.watermark(name, key)` checks liveness without recomputing. |
...
A fold is a derived view computed from a thread's (or the whole substrate's) records — the turn timeline, thread state, trust, and more. Read any of them through one method:
...
- **Fail-open** — record reads (`query`, `queryThread`, `richQuery`, `health`, `folds.resolve`) return an empty result on a *transient* failure (a briefly-unavailable daemon, a `5xx`, an auth blip), so a UI never crashes on backend trouble.
