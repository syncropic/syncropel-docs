...
`spl audit export` emits JSON Lines (one record per line) for security-relevant records from the engine's record log. Categories:
...
| `decision` | Decision-gate records — the request plus its answer (`body.verdict` accept/reject) | Reviewer approvals, dispatch verdicts |
...
**HTTP middleware permission denials are not records today.** They're `tracing::warn!` events in the instance log (`~/.syncro/logs/spl.log`) with a structured `PERMISSION DENIED` message. To get a complete security view in your SIEM, pipe the instance log alongside `spl audit export`. Promoting denials to first-class audit records is planned for a future release; for now, the two streams are complementary.

The other pre-existing limitation: `spl audit export` enumerates threads client-side and fetches records per thread. For stores with thousands of threads this becomes slow. Pass `--thread <id>` to scope when you can. A server-side `/v1/audit/export` endpoint that pushes filtering into SQL is planned for a future release.
...
```text
index=security sourcetype=syncropel:audit category=decision
...
```text
# All decision-gate rejections in the last 24h
index=security sourcetype=syncropel:audit category=decision record.body.verdict=reject
...
```
category: decision AND record.body.verdict: reject
```
...
```text
{job="syncropel-audit", category="decision"} | json | record.body.verdict="reject"
```
...
```
service:syncropel @category:decision @record.body.verdict:reject
```
...
```bash
# All decision-gate rejections this week
zcat /var/log/spl-audit/audit-*.jsonl.gz /var/log/spl-audit/*.jsonl 2>/dev/null \
    | jq -c 'select(.category == "decision" and .record.body.verdict == "reject")'
...
Filter for permission denials in the instance log — these are NOT in audit export today:

```text
index=security sourcetype=syncropel:instance "PERMISSION DENIED"
```
...
- [ ] Tested filter for each of the 4 categories (system, decision, dispatch, governance)
- [ ] Instance log also being shipped (for permission denials)
...
1. **Permission denials as first-class records** — the HTTP middleware will emit `did:sync:system:auth` records on every denial, making them appear in `spl audit export --categories governance`. Removes the need to ship the instance log separately for security purposes.
...
- [Operator Runbook](/docs/operate/runbook) — instance-log filtering for permission denials
