...
- **Not adversarial protection.** The filter runs on the source side, so it requires that the source instance is honest. For protection against a compromised source, use namespace-based access control on who can reach the workspace in the first place.
...
- **[Federation quickstart](./federation)** — pair two workspaces end-to-end
