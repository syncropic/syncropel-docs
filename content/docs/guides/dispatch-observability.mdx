...
**Why it exists**: bridges Syncropel records to the agent CLI's own per-project state on disk. When an agent-side bug or unusual tool-use pattern needs diagnosis, you need the raw agent-side log.
...
Records give you the shape of what happened. Logs give you the line-by-line narrative. The two are linked by the **sub-thread id** of the dispatch.

```bash
spl logs --trace-id th_8c51e9c8...
```

This reads the instance log and its rotated siblings, keeps the lines that belong to that dispatch, merges them chronologically, and prints them pretty-formatted.
...
4. **[Operator runbook](/docs/operate/runbook#dispatched-worker-died-unexpectedly-salvage-procedure)** — the salvage procedure for a partially-completed task.
