...
```bash
# 1. Author an admin allow rule FIRST (otherwise you'll lock yourself out)
spl config add-permission-rule \
  --name admin-write \
  --action allow \
  --priority 10000 \
  --expression 'current_actor() == "did:sync:user:YOUR_ADMIN_DID" && resource == "record_write"'
...
**Rollback trap**: `spl config permissions-disable` POSTs a LEARN record to `/v1/records` (resource = `record_write`). If your policy doesn't allow `record_write` for your actor, you can't disable enforcement from the CLI. The recovery path is a daemon restart with the `permissions_enabled` LEARN record removed from the store. Always author an admin allow rule before flipping enforcement on.
