...
description: Write rules, gates, and predicates using Syncropel's canonical expression language — one syntax for triggers, routing, preconditions, fold rules, health checks, AITL, and permissions.
---
...
| `routing` | Routing rules | `record`, `thread()`, `system`, `trust()`, `task()`, `now()`, `current_actor()` |
...
| `health_check` | `spl health check` predicates | `system`, `tasks`, `trust()`, `now()`, `current_actor()` |
| `aitl` | Intelligence proposal disposition | `record`, `proposal_confidence`, `meta_trust`, `system`, `now()`, `current_actor()` |
| `permission` | API request gating | `record`, `current_actor()`, `resource`, `trust()`, `task()`, `system`, `now()` |
