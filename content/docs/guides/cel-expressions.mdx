...
description: Write rules, gates, and predicates using Syncropel's canonical expression language — one syntax for triggers, routing, preconditions, fold rules, health checks, AITL, permissions, and (v0.11+) fan-out join predicates.
---
...
See the full specification in [§10/20 Expression Language](https://github.com/syncropic/syncropel-spec/blob/main/10-engine/20-expression-language.md) and the adoption rationale in [ADR-019](https://github.com/syncropic/syncropel-spec/blob/main/15-decisions/ADR-019-cel-expression-language.md).
...
| `routing` | Routing rules | `record`, `thread()`, `system`, `trust()`, `task()`, `now()`, `current_actor()`, `fanout_status()` *(v0.11+)* |
...
| `health_check` | `spl health check` predicates | `system`, `tasks`, `trust()`, `now()`, `current_actor()`, `fleet_instances_live()` *(v0.11+)* |
| `aitl` | Intelligence proposal disposition | `record`, `proposal_confidence`, `meta_trust`, `system`, `now()`, `current_actor()` |
| `permission` | API request gating | `record`, `current_actor()`, `resource`, `trust()`, `task()`, `system`, `now()`, `fleet_frozen_namespaces()` / `fleet_emergency_active()` *(v0.11+)* |
| `join` *(v0.11+)* | Fan-out barrier — when is a parent INTEND satisfied? | `children`, `parent`, `now()` |
...
```
task("SKL-0163").status        // string: "approved", "blocked", "active", etc.
task("SKL-0163").priority      // string: "critical", "high", etc.
task("SKL-0163").assigned_to   // string: actor DID
task("SKL-0163").age_secs      // int: seconds since creation
task("SKL-0163").cost_usd      // double
task("SKL-0163").exists        // bool: false for unknown aliases
```

Defensive check: `task("SKL-0163").exists && task("SKL-0163").status == "approved"`.
...
`current_namespace()` was added in v0.10.0 alongside the [namespace hierarchy](/docs/reference/cli#namespaces-v0100). Returns the namespace ID the current evaluation is scoped to (defaults to `"default"` when unset). Use `.startsWith("parent/")` to match an entire subtree of the 5-level hierarchy. Per-record namespace is accessible via `record.body.namespace`.
...
### Fleet bindings (v0.11+)

Added with ADR-021 (instance registry + kill switch) and ADR-022 (task fan-out). Scoped to specific contexts to avoid cross-context leakage.
...
### `children` (Join context only, v0.11+)
...
spl task add "ship v1.0" \
  --alias REL-0001 \
  --expression 'task("SKL-0163").status == "approved" && task("SKL-0164").status == "approved" && trust("did:sync:agent:dev", "code") > 0.5'
```

The shortcut `--depends-on SKL-0163,SKL-0164` compiles to an equivalent CEL expression internally, so you can mix the legacy sugar with the expression form.
...
# 3. Finally, enable enforcement. As of v0.9.2 a pre-flight runs first
...
**Pre-flight check (v0.9.2+)**: `spl config permissions-enable` simulates the disable path before flipping the switch. It refuses with a clear error and prints the exact `add-permission-rule` invocation needed when your loaded rules don't cover `(record_write, thread_read, config_read)` for the daemon's `identity.actor`.
...
| Preconditions | `--depends-on SKL-0163,SKL-0164` | `--expression 'task("SKL-0163").status == "approved" && task("SKL-0164").status == "approved"'` |
...
- **Spec**: [§10/20 Expression Language](https://github.com/syncropic/syncropel-spec/blob/main/10-engine/20-expression-language.md) — normative reference with test vectors
- **ADR**: [ADR-019 CEL Adoption](https://github.com/syncropic/syncropel-spec/blob/main/15-decisions/ADR-019-cel-expression-language.md) — decision rationale
