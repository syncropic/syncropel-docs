...
Syncropel uses **CEL (Common Expression Language)** as its canonical decision language. Every decision point in the system — event triggers, routing rules, task preconditions, status fold, health checks, the decision gate (the `aitl` evaluation context), and permissions — accepts CEL expressions. Write the predicate once; it works the same way everywhere.
