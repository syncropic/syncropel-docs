...
Now enable enforcement (with the v0.10.0+ pre-flight check that prevents the lockout trap):
...
spl config add-permission-rule \
  ...
  --action allow \
  --priority 10000 \
  --expression 'current_actor() == "did:sync:user:dpwanjala" && (resource == "record_write" || resource == "thread_read" || resource == "config_read")'
...
Use the v0.10.0 emergency unlock — see [Emergency Recovery](/docs/reference/cli#emergency-recovery-v0100):
...
## What's NOT yet enforced (v0.10.0 honest scope)

The narrowing guarantee in v0.10.0 enforces **existence** — every ancestor in the chain must be in the registry and Active. It does NOT yet enforce:

- **Capability subsets**: a record can claim any `body.dial`, `body.primitives`, etc. The daemon does not yet check that those capabilities are a subset of what the parent namespace's policy allows. (Tracked as [Namespace sub-task 8: policy YAML parser](https://github.com/syncropic/syncropel-core/blob/main/docs/namespace-spec-alignment.md).)
- **Store-level partitioning**: records are tagged with `body.namespace` but the SQLite store does not yet index by namespace, and read queries return records across all namespaces a caller can reach. (Tracked as [Namespace sub-task 9: store-level indexing](https://github.com/syncropic/syncropel-core/blob/main/docs/namespace-spec-alignment.md).)

For a single-tenant deployment or trusted-actor multi-tenant setup, v0.10.0's HTTP-boundary enforcement is sufficient. For untrusted multi-tenant where a customer might try to inject records claiming another customer's namespace, you also want the policy YAML + store-level isolation gaps closed before going live.

## Reference

- **Spec**: [§08-governance/04 Namespaces](https://github.com/syncropic/syncropel-spec/blob/main/08-governance/04-namespaces.md) — normative source
- **Implementation alignment**: [`docs/namespace-spec-alignment.md`](https://github.com/syncropic/syncropel-core/blob/main/docs/namespace-spec-alignment.md) — what's done, what's deferred, scorecard per spec section
