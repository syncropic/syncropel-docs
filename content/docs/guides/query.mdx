...
- **`limit`**: default 100, clamped to 1000. Asking for more returns 1000 rows and `capped: true`; read `capped` to know a page is partial, and never compare `matched_total` (the filter's count before disclosure) with the number of rows you received.
