...
Query lets you ask the daemon: _"give me every record where X"_, with X expressed as a structured filter document. The filter runs server-side — nothing streams over the wire except the matches. This is the right tool for any query that touches more than one thread, any filter over body shape (`body.kind`, `body.priority`, `body._refs.*`), or any workload big enough that client-side filtering wastes bandwidth.
