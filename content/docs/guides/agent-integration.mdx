...
The integration shape is the same: register an actor, bind a CLI adapter, point it at the binary. The engine doesn't care which agent — it just spawns the subprocess and parses the output stream.
...
```bash
spl dispatch send dev "Fix the failing authentication tests" \
  --budget 1.0 \
  --timeout 600
```
...
When an AI agent runs on a different machine than the `spl serve` instance — e.g. a workflow agent on a CI runner, or a coding agent reaching back to your laptop — the agent needs to authenticate over HTTP. See [Authentication & Service Accounts](/docs/operate/keyring) for the pattern:
...
An instance-level service account is the right shape for standing instance services — it survives personnel changes by design. For an agent that acts as an extension of a *person's* authority (their budget, their scopes, dying with their membership), see [Agent credentials](/docs/guides/agent-credentials) for the two paths and how to choose.
