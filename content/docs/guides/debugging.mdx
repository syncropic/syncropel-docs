...
Now you can see why the task isn't approved: there's no KNOW record from a *different actor* with a `verdict` field. The `spl task done` from dev moved the task to review, but no evaluator has approved it yet. The fix: have a distinct reviewer actor run `spl task approve SKL-0042`.
