...
description: When something isn't doing what you expect — instance won't respond, a task is stuck in the wrong status, a record didn't route — this is the order to reach for tools.
---

## When to use this guide

You're staring at output you don't understand. A `spl status` returned something wrong, a task is in a status you didn't expect, a record didn't trigger the routing rule you thought it would, or the instance isn't responding at all. Before you start grepping logs, work through these tools in order.
...
```
1. spl doctor            ─── is the instance even healthy?
        ↓
...
5. spl audit export      ─── what security-relevant events fired recently?
        ↓
6. tail ~/.syncro/logs/spl.log  ─── last resort: read raw instance output
```
...
## Step 1: `spl doctor` — is the instance healthy?

If you don't know whether the problem is in the instance or your understanding of the instance, run `spl doctor` first. It executes 7 read-only checks and prints PASS/WARN/FAIL with a short reason for each:
...
spl doctor
  ...
  ✓ instance reachable          <version> on sqlite:///home/you/.syncro/hub.db
  ✓ pid file                  /home/you/.syncro/run/spl.pid → live spl serve (pid 264247)
  ✓ store has records         4690 records across 232 threads
  ✓ store path agreement      instance reports sqlite:///home/you/.syncro/hub.db
...
**instance reachable** — `GET /health` returns 200. If this fails, the instance is not running OR not bound to localhost:9100 OR you're pointing `SPL_SERVE_URL` somewhere wrong.

**pid file** — `~/.syncro/run/spl.pid` exists, points at a live process, and that process's `/proc/<pid>/cmdline` contains "spl serve". A WARN here means the instance is responding but the PID file is missing — `spl stop` will not be able to find it. See [recovering an orphaned instance](/docs/operate/runbook#when-spl-stop-says-not-running-but-the-instance-clearly-is).

**store has records** — the store is non-empty. A WARN means it's empty (fresh install or a just-deleted store). A FAIL means the store query itself errored.

**store path agreement** — the instance's reported store URL matches what the config file says it should be. A WARN here means either the instance was started with an explicit `--store` override, or two instances are running against different stores and you're talking to the wrong one.

**intelligence** — the engine's intelligence agent's enabled state and model. Disabled is fine if you didn't set up an API key; if you did and this says disabled, your key isn't being read.
...
**expression cache** — the CEL expression cache stats. A cold cache (0 entries) is normal on a freshly-restarted instance. A WARN means the hit rate is suspiciously low (under 50%) which could indicate the instance is recompiling expressions every evaluation.
...
These are read-only summary commands. Together they tell you "the instance thinks the world looks like X". If X doesn't match what you expected, you've narrowed the problem to a single task, thread, or actor.
...
The first argument can be a task alias OR a literal thread ID. For non-task threads (e.g. `th_engine_config`, an decision thread, a dispatch thread) you'll see the same record-by-record fold trace. Useful for understanding why a routing rule fired or didn't fire, or what decision-gate decisions have been made on a particular intelligence proposal.

```bash
spl debug replay th_engine_config | head -10
spl debug replay $(spl thread list | grep decision | head -1 | awk '{print $1}')
```
...
This emits one JSON object per line for every security-relevant record in the last hour: system actor writes, decision-gate decisions, dispatch outcomes, governance events. Each line carries a `category` tag so you can filter:

```bash
spl audit export --since 1h --categories decision
...
HTTP middleware permission denials are currently `tracing::warn!` events in the instance log, not records. They show up in `~/.syncro/logs/spl.log` filtered with `grep "PERMISSION DENIED"` but won't appear in `spl audit export`. Promoting them to first-class audit records is tracked as a follow-up — for now, log + audit are two complementary streams.

## Step 6: read the instance log
...
If the rule loads, compiles, evaluates true on a sample, but isn't firing on real records, check the instance log for `reload_config: processing LEARN record` to confirm the rule was actually reloaded after you added it.

### "The instance won't start"

```bash
spl doctor                       # what's the current state?
tail -50 ~/.syncro/logs/spl.log  # what was the last thing the instance said?
ls -la ~/.syncro/run/            # is there a stale PID file?
```

Most start failures are: port already in use, stale PID file, or a corrupt SQLite store. The runbook covers each: see [Instance lifecycle](/docs/operate/runbook#instance-lifecycle).
...
spl audit export --since 1h --categories decision                # decision-gate decisions in the same window
```

If you've locked yourself out, see [`spl config permissions-unlock`](/docs/reference/cli#emergency-recovery-v0100).

### "I deleted the record store by accident"

Don't restart the instance. The destructive backup gotcha will overwrite your good backup the moment a fresh instance starts. See [Backup discipline](/docs/operate/runbook#backup-discipline) — restore from off-host first.
...
- **Trust score derivation traces** — `spl trust` shows current scores, but not the per-record evidence chain that produced them. A `spl debug trust <actor> <domain>` command is planned for a future release; for now, read the verdict records on the actor's trust thread directly.
- **Dispatch internals** — when a dispatch fails, you see the failure in the task thread, but not the adapter-side timings, retries, or sub-thread tool calls. Pull these from the instance log filtered by `target=syncropel_engine::dispatch`.
