...
The four debugging commands shipped in v0.10.0 (`spl doctor`, `spl debug replay`, `spl debug thread-diff`, `spl audit export`) cover ~95% of the failure modes you'll hit operating Syncropel. Each one is read-only — none of them mutate state, so you can use them freely without worrying about making things worse.
...
spl doctor
  ...
  ✓ daemon reachable          v0.10.0 on sqlite:///home/you/.syncro/hub.db
...
spl namespace list             # registered namespaces (v0.10.0+)
```
...
You found the problematic task. `spl task show SKL-0042` says it's in status `review`, but you expected `approved`. Why?

Run replay against the task's thread:

```bash
spl debug replay SKL-0042
```
...
spl debug replay th_be084400f080b84313c90321ffda6eae8ffca3b51c05abc13ebbf7403a609f86
  ...
  CLOCK  ACT    ACTOR                               STATUS       BODY
      0  INTEND did:sync:user:dpwanjala             inbox        goal=Fix the auth bug
...
Now you can see why the task isn't approved: there's no KNOW record from a *different actor* with a `verdict` field. The `spl task done` from dev moved the task to review, but no evaluator (director or another distinct actor) has approved it yet. The fix: have a director run `spl task approve SKL-0042`.
...
```bash
spl debug thread-diff SKL-0042 SKL-0035
```
...
- The good one (SKL-0035) has 3 participants vs 2. Likely the missing participant is the evaluator.
...
### What's NOT in audit export today (v0.10.0)
...
Look for a KNOW record with `body.verdict = "accept"` from an actor different from the one who claimed completion. If none exists, the task hasn't been evaluated yet. The fold rule [requires distinct evaluator and executor](https://github.com/syncropic/syncropel-spec/blob/main/12-experience/08-task-management.md) — that's the separation-of-duties gate.
