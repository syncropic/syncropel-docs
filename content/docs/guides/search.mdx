...
All three talk to the same embedding provider — configured once per daemon, used by every search. The **Ollama** provider (local, zero-cost) ships today; additional hosted providers land in later releases. If you just need text search and don't want to configure a provider, use [Find](./find) — it works everywhere with zero setup.
