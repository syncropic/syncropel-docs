...
Records that existed _before_ you configured a provider do **not** get back-filled automatically. To embed existing records, the easiest path today is re-ingest from a backup (see [Backup & recovery](/docs/guides/backup-restore)). A dedicated `spl embed backfill` command is on the roadmap.
