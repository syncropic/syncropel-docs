...
Semantic search answers the question _"give me records that mean something like X"_, where X is free-text and "means like" is cosine similarity in an embedding space. It complements [Query](./query): query is the right tool when you know the shape of what you want (`body.kind`, `body.priority`, exact thread); search is the right tool when you only know the _idea_ and want records nearby that idea regardless of surface form.
