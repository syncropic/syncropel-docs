...
A instance only publishes the federation manifest if it has a cryptographic identity configured:
...
5. **Opt-in advertising.** A instance with an identity keypair decides whether to populate `advertises.kinds`. Empty is acceptable — the peer is reachable for explicit pairing but does not broadcast a menu.
