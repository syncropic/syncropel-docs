...
description: How to ship a Syncropel iframe extension today — naming, hosting, capability declarations, security expectations, versioning. What is available now.
---
...
| [Discovery](#discovery-today-and-tomorrow) | Opt-in URLs. |
...
- Don't squat. If you don't own the domain, don't use it as a publisher.
...
| Host type | Setup cost | Best for |
|---|---|---|
| **Static hosting platform** (auto-deploy from Git) | ~5 minutes | Most extensions; many offer a free tier and a global CDN |
| **Git-integrated host with preview deploys** | ~5 minutes | Per-branch preview deploys are handy during extension dev |
| **Static hosting from a public repo** | ~10 minutes | Open-source extensions; CORS works fine |
| **Self-hosted** (object storage + a CDN, or your own reverse proxy) | Variable | Hosts that need network controls |
...
| `publisher.did` | no | A `did:web:<domain>` if you're advertising one. |
...
This works. It scales to dozens of extensions per workspace.
