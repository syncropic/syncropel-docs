...
runbook, the record is what you check in. Drop a JSON file like the
example at the top of this page into your seed records directory, then
POST it through `/v1/records` with the operator's DID as `actor`. The
seed loader in `syncropic-ops/services/graph/seed/seed.sh` is one
reference shape; the `core.site.v1` body in
`syncropic-ops/services/graph/seed/records/10-site.json` is the
record that runs `graph.syncropel.com` itself.
