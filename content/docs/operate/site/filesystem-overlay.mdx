...
operator-supplied assets — all without modifying the kernel or
...
These flow into the response between the kernel-emitted `<meta>` tags
and the closing `</head>`.

## `assets/` — operator-supplied files at `/site/local/<name>`

Anything under `~/.syncro/site/assets/` is served by the kernel at
...
references (`/site/local/logo.png?v=2`). The kernel does not version
