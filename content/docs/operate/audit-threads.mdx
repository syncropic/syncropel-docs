...
| `th_audit_snapshots` | `core.snapshot.event.v1` | SKL-0676 | LEARN per snapshot create / restore; criterion C2 (security-sensitive instance-level operation) |
| `th_audit_tokens` | `core.token_lifecycle_event.v1` | ADR-083 D4, SKL-0677 | LEARN per token mint / revoke + SA create / delete / rotate; mirrors `api_tokens_thread(<ns>)` / `service_accounts_thread(<ns>)` |
