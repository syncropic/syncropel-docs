...
This page is for the operator running two (or more) Syncropel daemons that need to talk to each other. Single-instance operators don't need pairs — pairs exist specifically to bridge separate instances under separate identities, with consent and credentials negotiated explicitly.
...
- **You're bridging a hosted instance and a local one** — say, `<label>.syncropel.app` for the agent stack and a laptop daemon for capture-and-emit.
