...
description: Enable bearer-token authentication, create service accounts, pair devices, and manage token lifecycle. Bearer-token auth is enforced by default on every spl serve instance.
---

<Callout type="info">
**`spl serve` is default-secure.** `auth.required = true` is the default — every instance rejects unauthenticated requests out of the box. To bootstrap: create the first service account, save the token, and every `spl` command works from there.
</Callout>

## TL;DR

- On a **fresh install**: bootstrap your first service account, save the token, and every `spl` command Just Works. Five-step [Quick-start](#3-quick-start-enabling-auth-on-a-remote-host) below.
...
Bearer-token authentication protects every HTTP request. Without it, `spl serve` would be catastrophic the moment the instance was reachable from any other machine — over LAN, a private network, a tunnel, a reverse proxy, or an accidentally-bound `0.0.0.0` interface. The authentication layer closes that hole:
...
Assume you have `spl serve` running on `localhost:9100`, and you've exposed it to another device at `https://your-host.example.com:9100` (via reverse proxy, private network, or tunnel of your choice).
...
The instance reloads `th_engine_config`, picks up the new setting, and starts enforcing.
...
## 4. Pairing a phone (or any device) to the instance
...
**Choose `--url` carefully.** It's the URL the phone will call — not your instance's `localhost:9100`. In order of typical preference:
...
If your token is lost, your laptop is offline from the server, or you locked yourself out of your own instance:
...
This starts the instance force-bound to `127.0.0.1` only (no external reachability regardless of `--host`) with auth enforcement disabled. You regain `spl` CLI access for cleanup:
...
The instance logs a WARN on every startup when `--insecure-localhost` is active. This is intentional — it should never be the default state.
...
If that says "invalid format," re-export `SPL_TOKEN` from your shell config. If it says "token_id not found," the token has been revoked or the instance's index hasn't picked it up yet (rare; restart instance).
...
- [Federation pair guide](/docs/guides/federation-pair) — how bearer tokens compose with Ed25519 signatures for peer-to-peer sync
