...
description: Enable bearer-token authentication, create service accounts, pair devices, and manage token lifecycle. Covers the authentication substrate shipped in v0.15 and when you need it.
---
...
- On **any remote or shared host** (cloud VM, private network, reverse proxy, tunnel): enable bearer-token auth. Five steps in [Quick-start](#3-quick-start-enabling-auth-on-a-remote-host) below.
...
Until v0.16 ships, the `spl` CLI does not auto-inject the bearer token. Your choices:
...
This is expected until v0.16 lands. Workarounds:
