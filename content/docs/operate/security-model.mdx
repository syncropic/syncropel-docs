...
| A work loop reading the provider credential or the operator's bearer | ✓ (sandboxed transport, opt-in) | separate isolated process, no network, no view of the instance home, one scoped credential handed to it at start |
...
Host tools (`bash` and the filesystem set) are wired for the operator's own machine only. The rollback for the whole transport is one config record; see [The sandboxed transport](./sandboxed-transport).
