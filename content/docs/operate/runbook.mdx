...
- **`~/.syncro-data/`**: task content files (the rich-Markdown bodies for each task — addressed by their alias, e.g., `MY-001.md`) and the alias-to-thread mapping. The metadata (titles, statuses, hashes) lives in records on `hub.db`; the canonical content files live here on disk and are referenced by path during `spl task dispatch` (briefs are passed by file path since v0.27.1, not inlined — see [the v0.27.1 changelog](https://github.com/syncropic/syncropel-core/blob/main/CHANGELOG.md#0271--2026-05-01)). Backing up `hub.db` without `~/.syncro-data/` leaves task content stranded. Both belong in your snapshot.
