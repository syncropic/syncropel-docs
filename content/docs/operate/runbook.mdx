...
### When `spl stop` says "not running" but the daemon clearly is
...
**The startup backup is destructive.** Every `spl start` invocation overwrites the backup file with a snapshot of the current `hub.db`, **even if the current `hub.db` is empty, corrupt, or wrong.**
...
```bash
spl stop
spl start  # rebuild_from_store() runs on startup
...
spl start                # restart — enforcement is off
```
...
SPL_ACTOR=did:sync:agent:director spl task approve <task-id> --domain code --notes "..."
```
...
4. **Restart**: `SYNCROPEL_HOME=~/.syncro-worker-a spl start --port 9201`. The promoted instance now holds the registry.
