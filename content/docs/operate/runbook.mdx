...
Schedule a periodic off-host backup of BOTH `~/.local/share/syncropel/backups/<instance-key>/hub.db.bak` (records, trust, config) AND `~/.syncro-data/` (task content files and alias map). The daemon auto-backs up only the former; the latter is referenced by path during dispatch but not auto-snapshotted.
...
- **`~/.syncro-data/`**: task content files (the rich-Markdown bodies for each `SKL-XXXX` task) and the alias-to-thread mapping. The metadata (titles, statuses, hashes) lives in records on `hub.db`; the canonical content files live here on disk and are referenced by path during `spl task dispatch` (see [SKL-0536](https://github.com/syncropic/syncropel-core/blob/main/CHANGELOG.md#0271--2026-05-01) — briefs are passed by file path, not inlined). Backing up `hub.db` without `~/.syncro-data/` leaves task content stranded. Both belong in your snapshot.
...
When you dispatch a worker — `spl task dispatch <SKL>`, `spl run <goal>`, or any other path that spawns a subprocess — the daemon emits one of three log lines at spawn:
...
```bash
cd /path/to/syncropel-core-SKL-XXXX
git status -s
```
...
- **Partial** — read the SKL brief at `~/.syncro-data/tasks/SKL-XXXX.md`, identify the missing acceptance-criteria items, complete them in the worktree, then proceed.
...
```bash
export CARGO_RUSTC_WRAPPER=sccache
...
All four must pass. The salvage is only as trustworthy as the gate.

### Step 5 — Commit on the task branch

The worktree is already on `task/SKL-XXXX`. Commit there:

```bash
git add <specific-files>
git commit -m "feat(scope): <change> (SKL-XXXX)
...
```bash
cd /path/to/syncropel-core   # main worktree
git merge task/SKL-XXXX --ff-only       # or `git cherry-pick <hash>` if main moved
cargo fmt --all -- --check
...
### Step 7 — Reopen + dev-done + director-approve

The original worker's `dispatch_complete` left the task in `failed` status. Reopen and walk it through the lifecycle:

```bash
spl task reopen SKL-XXXX --reason "salvaged in-session after worker SIGKILL"
SPL_ACTOR=did:sync:agent:dev      spl task done    SKL-XXXX --summary "..." --domain code
SPL_ACTOR=did:sync:agent:director spl task approve SKL-XXXX --domain code --notes "..."
```

Reference the salvage commit hash + workspace gate result in the director's `--notes` so trust evidence accumulates correctly.

### Step 8 — Clean up the worktree

```bash
cd /path/to/syncropel-core
git worktree remove syncropel-core-SKL-XXXX --force
```
...
- **Close idle claude sessions before dispatching parallel.** Each background `claude --dangerously-skip-permissions` session consumes 200-500MB resident.
