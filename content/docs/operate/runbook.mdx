...
- **Routing rules, fold rules, health checks, decision policies, permission rules**: all stored as LEARN records. Will rebuild.
...
### Symptom: the record store was deleted by accident
...
If the instance runs under systemd, the same sequence is `systemctl --user stop spl-serve`, install, `systemctl --user start spl-serve`. Stop first in every case: a live copy over the running binary is refused (`Text file busy`), and a work loop's worker is started from the daemon's own binary, so an already-running worker keeps the version it started with until the daemon restarts.

The daemon's process is protected, so a same-user shell cannot read its process entries; to confirm which binary a running daemon is, use `sudo -n readlink /proc/<pid>/exe`. The daemon also produces no core dump. Both are the hardening working, not a fault.

Work loops for every non-operator principal run through the sandboxed transport. To stop one at its next step boundary, `POST /v1/runs/{thread}/events` with `{"type":"stop"}`; the run's next tool call is answered with a stop verdict, the run ends `cancelled`, and its worker is reaped. The rollback for the transport itself is one config record per tier (`work_loop_transport` for the operator, `work_loop_transport_members` for everyone else); see [The sandboxed transport](./sandboxed-transport).
...
When you dispatch a worker — `spl task dispatch <task>`, `spl run <goal>`, or any other path that spawns a subprocess — the instance emits one of three log lines at spawn:
...
The lines are visible in `~/.syncro/logs/spl.log` (filter for `dispatch pre-flight`) and in `journalctl` if you're running under systemd. Watch host memory: when the instance reports memory pressure, do not start more parallel workers, free memory (idle browser tabs, idle agent sessions, anything not load-bearing), and drop to sequential dispatch (`max_concurrent = 1` on the adapter) before you spawn more. If a worker was killed during the pressure window, follow the salvage procedure below.

The pre-flight check runs on Linux; on macOS and Windows, watch the OS process monitor instead.
...
- **Partial** — read the task brief (`spl task show <task-id>`), identify the missing acceptance-criteria items, complete them in the worktree, then proceed.

### Step 4 — Commit on the task branch
...
inventory>; only <missing-item> needed adding."
```
...
# Run your project's own test suite here
git push origin main
```

### Step 6 — Reopen + done + approve
...
Reference the salvage commit hash and test result in the reviewer's `--notes` so trust evidence accumulates correctly.
...
- **Cap parallel dispatch by available memory.** On a host already running a few agent sessions and `spl serve`, even `max_concurrent = 2` may be too many. If host memory is tight, drop to `max_concurrent = 1`.
...
This section covers the operations you run day-2 on a fleet. For a first walkthrough, see [Your first fan-out](/docs/tutorials/first-fanout).
