...
<Callout type="info">
**Audience**: operators who want to verify v0.11.0's parallel velocity claim against their own workloads, or who want a repeatable benchmark to track regressions across Syncropel versions. This page is the actionable companion to the [dogfood measurement methodology](https://github.com/syncropic/syncropel-research/blob/main/docs/compute-plane/09-v0.11-dogfood-measurement-methodology.md) used internally for the v0.11.0 release gate.
</Callout>
...
On a different session of the same task (same day, same local state — you're comparing equivalent work), run the fan-out version:

```bash
# Create a copy of the task with body.subtasks declared
spl task add "<same goal>" --body-file <task>-parallel.md

# Then fan out
spl task fan-out <task-alias>
```

The body file declares the subtasks and join predicate:

```yaml
subtasks:
  - goal: "subtask 1 goal"
    target: worker-a
    budget_usd: <reasonable>
    timeout_secs: 1800
...
- [Dogfood Measurement Methodology](https://github.com/syncropic/syncropel-research/blob/main/docs/compute-plane/09-v0.11-dogfood-measurement-methodology.md) — the full research document this guide distills
