---
title: spl doctor
description: Top-down diagnostic that audits a daemon's filesystem state, PID files, ports, config, and permissions. Run this first when something feels wrong.
---

<Callout type="info">
**Since v0.10** (expanded v0.16.1 for file-mode checks).
</Callout>

## What it does

`spl doctor` is the "something's wrong, where do I start?" command. It
runs 7 sanity checks against a running or intended daemon and reports
pass / warn / fail per check. Exit code reflects the worst finding:

- `0` — all checks pass
- `1` — one or more warnings (non-fatal; daemon likely works but is
  misconfigured)
...
| 4 | **Bind port reachable** | `curl /health` on configured port returns 200 | Restart daemon; check firewall |
...
| 7 | **File mode audit** (v0.16.1+) | `~/.syncro/token` is `0600`, `config.toml` not world-readable, `secrets/*` is `0600` | `chmod 0600 <file>` |
...
- **[Runbook](/docs/operate/runbook)** — operational procedures for
  daemon orphans, data-dir recovery, and upgrade drills
