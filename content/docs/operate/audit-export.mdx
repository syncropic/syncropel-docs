...
| `decision` | Decision-gate records — request + answer (`body.verdict` accept/reject) — reviewer approvals, dispatch verdicts |
...
# Last 24 hours, decision-gate + dispatch only, piped to jq
spl audit export --since 24h --categories decision,dispatch | jq '.'
...
instances — the output can be large.
...
```bash
# decision-gate only → review dashboard
spl audit export --since 24h --categories decision \
...
- The command runs against the instance over the Unix socket (prod) or
