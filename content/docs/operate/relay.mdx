...
Download the binary for your platform from [releases.syncropic.com](https://releases.syncropic.com/spl/latest/) (Linux x86_64 and aarch64 are built).
...
Envelopes default to 30 days. Senders can override per-envelope via the `ttl_seconds` field. Expired envelopes are pruned on demand rather than by a background sweep.
...
The in-memory queue has no durable state to back up. Back up the relay's configuration and unit files as you would any service.
...
# systemd
curl -fsSL https://releases.syncropic.com/spl/latest/syncropel-relay-linux-x86_64 \
