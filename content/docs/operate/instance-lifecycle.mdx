...
`spl init` creates `~/.syncro/`, generates a instance identity (or surfaces the existing one), and prints the next-step commands. It does not start the instance.
...
You should see a startup banner naming the bind address, the SQLite path, and the backup destination. `Ctrl-C` to stop. Once you're satisfied, run it as a instance:
...
For a instance that survives reboots, see [Keeping Your Instance Running](/docs/operate/persist-instance) — `systemd` user units on Linux, `launchd` plists on macOS, and Windows Service wrappers are all documented there.
