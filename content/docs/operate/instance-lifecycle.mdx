...
<Mermaid
  caption="syncropel-core crate dependency graph. Strict layering, no upward dependencies."
  chart={`flowchart TD
...
<Mermaid
  ...
  chart={`sequenceDiagram
    ...
    participant Auth as auth-proxy Worker
    participant Fly as Fly Machines API
    participant K as spl kernel<br/>(Firecracker microVM)

    U->>Site: sign in via Clerk
    Site->>Prov: POST /provision<br/>(Clerk JWT, label)
    Prov->>Prov: generate bootstrap secret<br/>+ KV{label → state}
    Prov->>Fly: create app + volume + secrets<br/>(SPL_BOOTSTRAP_TOKEN)
    Prov->>Fly: deploy machine (image, services 80/443)
    ...
    Auth->>Auth: verify Clerk JWT<br/>resolve KV{label → bearer}
