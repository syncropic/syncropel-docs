...
<Mermaid
  caption="The four kernel loops. State is fold(records); none of the loops mutate anything outside the record log."
  chart={`flowchart LR
    ...
    subgraph CRON["CRON (scheduled)"]
        ...
        Crys --> Aitl["AITL timeout sweep"]
    end`}
/>

The internal layers follow the same structure. Nothing above depends on anything below it; the kernel is portable along any of the dashed boundaries (Records / Algebra and Trust are portable to the browser).

<Mermaid
  caption="spl daemon internal layering. Strict layering, no upward dependencies."
  chart={`flowchart TD
...
For a daemon that survives reboots, see [Keeping Your Instance Running](/docs/operate/persist-instance) — `systemd` user units on Linux, `launchd` plists on macOS, and Windows Service wrappers are all documented there.

### Hosted (Syncropic-provisioned)

The hosted-instance flow is the same kernel as the self-hosted one — same binary, same record store, same APIs — running on Syncropic's managed platform. Customers don't manage the kernel directly; the provisioning service stands the instance up from a signed-in session and an edge auth layer brokers requests to the right per-customer instance.

<Mermaid
  caption="Self-service hosted instance provisioning. Same kernel, different boot ritual."
  chart={`sequenceDiagram
    ...
    participant K as spl kernel<br/>(hosted instance)
    ...
    K->>K: spl start<br/>read SPL_BOOTSTRAP_TOKEN
...
Once bootstrapped, treat `auth.required = true` as load-bearing. It is the kernel default and every shipping route enforces it (including the federation surface). Specifics live in [Authentication & Service Accounts](/docs/operate/keyring); the operational rules are:
...
> **CRITICAL — read this section twice.** Syncropel's startup backup is a safety net, not a backup system. It will not save you if you don't supplement it with off-host copies. The recovery drill that ships with the daemon source tree was written specifically because operators conflated the two.
...
This sends `SIGTERM`. The daemon flushes the SQLite WAL, closes the socket, and removes its PID file. If `--stop` reports "not running" but the port is still bound, see the [orphan-recovery section on the runbook](/docs/operate/runbook).
...
The three directories together capture everything: `~/.syncro/` is the daemon state and identity, `~/.syncro-data/` is task content + alias mappings, `~/.local/share/syncropel/` is the rolling backup history. Move the tarball off-host before deleting the originals.
