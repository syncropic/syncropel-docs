...
<Callout type="info">
**Status**: Code Mode is the third of three integration patterns documented in [AI Clients](/docs/integrate/ai-clients). Shipped with v0.17 "Exposure" (SKL-0317). Last updated 2026-04-21.
</Callout>
...
The Python SDK is async; wrap the work in an `async` function and `asyncio.run` it, or use `emit_sync` for synchronous harnesses. Every record needs a `body.kind` per ADR-034 — the SDK validates it before the network call so bad kinds fail fast.
...
The agent's context sees one line of output instead of 5-7 tool calls worth of chatter. For fully synchronous harnesses (Claude Code Interpreter, scripts), swap every `await client.emit(...)` for `client.emit_sync(...)` — same signature, no event loop required.
