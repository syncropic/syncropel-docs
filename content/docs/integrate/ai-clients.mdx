...
- `--flavor code|cursor|windsurf` — Cline only; tell the installer which VS Code host has the Cline extension
...
Add to `~/.config/zed/settings.json`. Zed calls MCP servers "context servers" and **requires** a `"source": "custom"` field — without it, Zed silently ignores the entry:
...
- `dispatch` — dispatch work to an actor via adapter routing (stdio transport only in v0.17)
- `fan_out` — fan-out work to multiple actors with a join predicate (stdio transport only in v0.17)

### Remote MCP over HTTP (experimental, v0.17+)

For hosted setups, sandboxed agents, or when you want MCP tools reachable over the network, `spl serve` also exposes MCP over HTTP at `/v1/mcp`. Speaks the same JSON-RPC 2.0 envelope as stdio; streaming SSE responses are not implemented in v0.17 (every tool returns a single JSON body fast enough).
...
**v0.17 limitation**: the `dispatch` and `fan_out` tools return an `isError: true` response over HTTP — they need the full drain-state + orchestration machinery and are stdio-only for this cut. Restoring full parity is tracked for v0.17.1. The other 5 tools work identically on both transports.
