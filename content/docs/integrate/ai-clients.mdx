...
description: Connect Syncropel to MCP-compatible AI clients including Claude Desktop, Claude Code, Cursor, Cline, Zed, Kiro, OpenCode, Gemini CLI, and Codex CLI. Three patterns — traditional MCP, shell-integrated, and programmatic — cover the major shapes of AI client integration.
---
...
| **Programmatic** | Any agent that writes code (code-execution-capable LLMs, edge-runtime agents, custom harnesses) | `@syncropel/sdk` (npm) or `syncropel` (PyPI) | A bearer token at `Client` construction |
...
- `find` — blended text search over file names, file contents, and conversations (see the [Find guide](/docs/guides/find))
...
All 8 tools are available on both stdio and HTTP transports today. The HTTP dispatch path stays leaner than `POST /v1/dispatch` directly (no budget admission layer, no lifecycle-event emission); clients that need the full production dispatch semantics should hit `POST /v1/dispatch` directly, not via MCP.
...
See the [Programmatic Agents guide](/docs/integrate/programmatic-agents) for worked examples in Python and TypeScript.

### When to choose this pattern

- Your agent harness supports code execution (any code-execution-capable LLM, a serverless platform, a custom Python/Node sandbox)
...
All three patterns authenticate against the Syncropel instance the same way: a bearer token. See the [security model](/docs/operate/security-model) for the full setup. Quick path:
...
- [Programmatic Agents guide](/docs/integrate/programmatic-agents) — deeper walk through Pattern 3 with full examples
