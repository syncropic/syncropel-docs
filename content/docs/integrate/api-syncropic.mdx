...
curl https://api.syncropic.com/v1/chat/completions \
  ...
  -d '{
    ...
    "stream": true,
    "messages": [
      {"role": "user", "content": "stream a short poem about substrate-as-coordination"}
    ]
  }'
```
...
- **Tokens-on-delivery accounting.** You're billed for what was actually streamed to your client, not what the upstream model produced. If you cancel the connection mid-stream, the audit record reflects the partial token count.
- **Audit records carry a `stream_state` field** on `core.api.usage.v1` — `"completed"` for normal stream end, `"client_disconnect"` if your client closed the connection before the model finished.
- **Tier gates run up-front.** Subscription, rate limit, and bearer cap are evaluated *before* the upstream connection opens; a 429 or 402 will surface as a normal HTTP response, not as an SSE error frame.
- **Anonymous tier supports streaming** via `gemini-flash-1.5`. Same per-IP throttle as non-streaming.
