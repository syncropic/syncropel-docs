...
description: Integrate Syncropel with agents that accomplish tasks by writing and executing code. One context round-trip for N operations. Works with any code-execution-capable LLM, edge-runtime agents, and any sandboxed code-generation harness.
---
...
- **Edge runtimes** running a work loop with fetch
...
And a Syncropel instance reachable from the sandbox's network:
...
## Quickstart — TypeScript, edge runtime
...
Deploy with your edge runtime's CLI. Every webhook handled emits 3 records. Zero LLM context round-trips per webhook.
...
This is intentional: your agent's core work shouldn't crash because a coordination record failed to reach the instance. The alternative (throwing exceptions) makes Syncropel a liability instead of an asset.
...
For unauthenticated local instances (dev mode), omit `api_key`.

For cloud agents, store the token in your platform's secret manager:
- **Edge runtimes**: store `SPL_TOKEN` as a secret with your platform's CLI
